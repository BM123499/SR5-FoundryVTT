...
export class MovementPrep {
    static prepareMovement(system: Actor.SystemOfType<'character' | 'critter' | 'spirit'>) {
        ...
        movement.walk.base = attributes.agility.value * (2 + modifiers['walk']) + new PartsList(movement.walk.mod).total;
        movement.run.base = attributes.agility.value * (4 + modifiers['run']) + new PartsList(movement.run.mod).total;
